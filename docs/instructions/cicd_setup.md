# Настройка CI/CD для автоматического развертывания

## Дата создания
2025-12-02

## Обзор

Данная документация описывает процесс настройки CI/CD (Continuous Integration / Continuous Deployment) для автоматического развертывания Telegram-бота на сервере с использованием GitHub Actions.

**Цели:**
- Автоматизация процесса развертывания
- Снижение риска ошибок при ручном деплое
- Быстрое развертывание новых версий
- Возможность отката к предыдущей версии
- Безопасное хранение секретов

---

## 1. Анализ возможностей GitHub Actions

### 1.1. Что такое GitHub Actions?

GitHub Actions — это встроенная платформа CI/CD от GitHub, которая позволяет автоматизировать процессы разработки, тестирования и развертывания.

**Преимущества:**
- ✅ Интеграция с GitHub репозиторием
- ✅ Бесплатно для публичных репозиториев и 2000 минут/месяц для приватных
- ✅ Простота настройки через YAML файлы
- ✅ Поддержка Docker и контейнеризации
- ✅ Безопасное хранение секретов через GitHub Secrets
- ✅ Возможность ручного запуска (manual trigger)
- ✅ Поддержка различных операционных систем и окружений

### 1.2. Возможности для нашего проекта

**Поддерживаемые сценарии:**
1. **Автоматическое развертывание** при push в ветку `main`
2. **Ручной запуск** (manual trigger) для владельца репозитория
3. **Сборка Docker-образа** на сервере
4. **Остановка и запуск контейнеров** через SSH
5. **Откат к предыдущей версии** через теги или ручной запуск

**Ограничения:**
- GitHub Actions не может напрямую управлять удаленным сервером
- Требуется SSH доступ к серверу для развертывания
- Для Docker registry (опционально) требуется дополнительная настройка

---

## 2. Архитектура CI/CD

### 2.1. Схема развертывания

```
[GitHub Repository]
    |
    | (push в main или manual trigger)
    |
[GitHub Actions Workflow]
    |
    | (SSH подключение)
    |
[Production Server]
    |
    | (docker compose)
    |
[Docker Containers]
    ├── bot (обновляется)
    ├── postgres (не обновляется)
    └── pgadmin (не обновляется)
```

### 2.2. Процесс развертывания

1. **Триггер**: Push в ветку `main` или ручной запуск
2. **Подключение к серверу**: SSH подключение с использованием секретов
3. **Обновление кода**: `git pull` на сервере
4. **Сборка образа**: `docker compose build bot`
5. **Остановка старого контейнера**: `docker compose stop bot`
6. **Запуск нового контейнера**: `docker compose up -d bot`
7. **Проверка статуса**: Проверка, что контейнер запущен успешно
8. **Уведомление**: (опционально) Отправка уведомления об успешном/неуспешном деплое

---

## 3. Настройка GitHub Secrets

### 3.1. Необходимые секреты

Для работы CI/CD необходимо настроить следующие секреты в GitHub:

1. **`DEPLOY_HOST`** — IP-адрес или доменное имя сервера
2. **`DEPLOY_USER`** — имя пользователя для SSH подключения 
3. **`DEPLOY_SSH_KEY`** — приватный SSH ключ для подключения к серверу (БЕЗ passphrase!)
4. **`DEPLOY_PATH`** — путь к директории проекта на сервере

**Опциональные секреты:**
- **`DEPLOY_PORT`** — SSH порт (по умолчанию: 22)
- **`TELEGRAM_NOTIFICATION_TOKEN`** — токен Telegram бота для уведомлений о деплое
- **`TELEGRAM_NOTIFICATION_CHAT_ID`** — ID чата для отправки уведомлений

### 3.2. Как добавить секреты в GitHub

1. Откройте репозиторий на GitHub
2. Перейдите в **Settings** → **Secrets and variables** → **Actions**
3. Нажмите **New repository secret**
4. Введите имя секрета (например, `DEPLOY_HOST`)
5. Введите значение секрета
6. Нажмите **Add secret**
7. Повторите для всех необходимых секретов

### 3.3. Генерация SSH ключа для деплоя

**⚠️ ВАЖНО:** Для CI/CD необходимо создать SSH ключ **БЕЗ ПАРОЛЯ (passphrase)**! GitHub Actions не может вводить пароль в неинтерактивном режиме.

**На сервере (подключитесь как root):**

```bash
# Создайте SSH ключ специально для CI/CD БЕЗ ПАРОЛЯ
# При запросе passphrase просто нажмите Enter (оставьте пустым)
ssh-keygen -t ed25519 -C "github-actions-deploy" -f ~/.ssh/github_actions_deploy -N ""

# Убедитесь, что директория .ssh существует и имеет правильные права
mkdir -p ~/.ssh
chmod 700 ~/.ssh

# Скопируйте публичный ключ в authorized_keys
cat ~/.ssh/github_actions_deploy.pub >> ~/.ssh/authorized_keys

# Установите правильные права доступа
chmod 600 ~/.ssh/authorized_keys
chmod 600 ~/.ssh/github_actions_deploy
chmod 644 ~/.ssh/github_actions_deploy.pub
```

**Проверка ключа (опционально):**

```bash
# Проверьте, что ключ создан без passphrase
ssh-keygen -y -f ~/.ssh/github_actions_deploy
# Должно вывести публичный ключ без запроса пароля
```

**В GitHub Secrets:**

1. Скопируйте содержимое приватного ключа:
   ```bash
   cat ~/.ssh/github_actions_deploy
   ```
2. Скопируйте **весь вывод**, включая строки `-----BEGIN OPENSSH PRIVATE KEY-----` и `-----END OPENSSH PRIVATE KEY-----`
3. Добавьте его в GitHub Secret `DEPLOY_SSH_KEY`:
   - Settings → Secrets and variables → Actions
   - New repository secret
   - Name: `DEPLOY_SSH_KEY`
   - Value: вставьте весь приватный ключ

**Важно:** 
- Никогда не коммитьте приватные SSH ключи в репозиторий!
- Ключ для CI/CD должен быть БЕЗ passphrase
- Используйте отдельный ключ только для CI/CD, не ваш личный ключ

---

## 4. Создание GitHub Actions Workflow

### 4.1. Структура файла workflow

Создайте файл `.github/workflows/deploy.yml` в корне репозитория:

```yaml
name: Deploy to Production

on:
  push:
    branches:
      - main
  workflow_dispatch:  # Позволяет запускать вручную

jobs:
  deploy:
    runs-on: ubuntu-latest
    
    steps:
      - name: Checkout code
        uses: actions/checkout@v4
      
      - name: Deploy to server
        uses: appleboy/ssh-action@v1.0.0
        with:
          host: ${{ secrets.DEPLOY_HOST }}
          username: ${{ secrets.DEPLOY_USER }}
          key: ${{ secrets.DEPLOY_SSH_KEY }}
          port: ${{ secrets.DEPLOY_PORT || 22 }}
          script: |
            cd ${{ secrets.DEPLOY_PATH }}
            git pull origin main
            docker compose build bot
            docker compose stop bot
            docker compose up -d bot
            docker compose ps
```

### 4.2. Полный пример workflow с улучшениями

Создайте файл `.github/workflows/deploy.yml`:

```yaml
name: Deploy to Production

on:
  push:
    branches:
      - main
  workflow_dispatch:  # Ручной запуск

env:
  DEPLOY_PATH: ${{ secrets.DEPLOY_PATH }}

jobs:
  deploy:
    runs-on: ubuntu-latest
    timeout-minutes: 15
    
    steps:
      - name: Checkout code
        uses: actions/checkout@v4
      
      - name: Deploy to server
        uses: appleboy/ssh-action@v1.0.0
        with:
          host: ${{ secrets.DEPLOY_HOST }}
          username: ${{ secrets.DEPLOY_USER }}
          key: ${{ secrets.DEPLOY_SSH_KEY }}
          port: ${{ secrets.DEPLOY_PORT || 22 }}
          script: |
            set -e  # Остановка при ошибке
            
            echo "🚀 Начало развертывания..."
            
            # Переход в директорию проекта
            cd ${{ secrets.DEPLOY_PATH }}
            
            # Сохранение текущей версии (для отката)
            CURRENT_VERSION=$(git rev-parse --short HEAD)
            echo "📌 Текущая версия: $CURRENT_VERSION"
            echo "$CURRENT_VERSION" > .deploy_version
            
            # Обновление кода
            echo "📥 Обновление кода из репозитория..."
            git fetch origin
            git reset --hard origin/main
            git clean -fd
            
            # Проверка изменений
            NEW_VERSION=$(git rev-parse --short HEAD)
            echo "📌 Новая версия: $NEW_VERSION"
            
            # Сборка Docker-образа
            echo "🔨 Сборка Docker-образа..."
            docker compose build bot
            
            # Остановка старого контейнера
            echo "⏹️ Остановка старого контейнера..."
            docker compose stop bot || true
            
            # Удаление старого контейнера (опционально)
            echo "🗑️ Удаление старого контейнера..."
            docker compose rm -f bot || true
            
            # Запуск нового контейнера
            echo "▶️ Запуск нового контейнера..."
            docker compose up -d bot
            
            # Ожидание запуска контейнера
            echo "⏳ Ожидание запуска контейнера..."
            sleep 5
            
            # Проверка статуса контейнера
            echo "✅ Проверка статуса контейнеров..."
            docker compose ps
            
            # Проверка логов на ошибки
            echo "📋 Проверка логов..."
            if docker compose logs --tail=50 bot | grep -i "error\|exception\|traceback"; then
              echo "⚠️ Обнаружены ошибки в логах!"
              docker compose logs --tail=100 bot
              exit 1
            fi
            
            echo "✅ Развертывание завершено успешно!"
            echo "📌 Версия: $NEW_VERSION"
      
      - name: Send notification (optional)
        if: always()
        uses: appleboy/telegram-action@v1.0.0
        with:
          to: ${{ secrets.TELEGRAM_NOTIFICATION_CHAT_ID }}
          token: ${{ secrets.TELEGRAM_NOTIFICATION_TOKEN }}
          message: |
            ${{ job.status == 'success' && '✅' || '❌' }} Деплой завершен
            
            Статус: ${{ job.status }}
            Версия: ${{ github.sha }}
            Коммит: ${{ github.event.head_commit.message }}
            Автор: ${{ github.event.head_commit.author.name }}
        env:
          TELEGRAM_CHAT_ID: ${{ secrets.TELEGRAM_NOTIFICATION_CHAT_ID }}
          TELEGRAM_TOKEN: ${{ secrets.TELEGRAM_NOTIFICATION_TOKEN }}
```

### 4.3. Важно: Использование Docker Compose V2

**⚠️ ВАЖНО:** Workflow использует команду `docker compose` (V2) с пробелом. Убедитесь, что на сервере установлен Docker Compose V2, а не старая версия `docker-compose` (V1).

Если на сервере установлена старая версия `docker-compose` (V1), установите Docker Compose V2 согласно инструкциям в разделе 5.1.

---

## 5. Настройка сервера для CI/CD

### 5.1. Предварительные требования

**На сервере должны быть установлены:**
- Git
- Docker
- Docker Compose V2 (команда `docker compose`)
- SSH сервер

**⚠️ ВАЖНО:** Проект использует **Docker Compose V2** (команда `docker compose` с пробелом). Старая версия `docker-compose` (V1) не поддерживается.

#### Установка Docker и Docker Compose V2

**Для Ubuntu/Debian:**

```bash
# Обновление пакетов
sudo apt update

# Установка зависимостей
sudo apt install -y ca-certificates curl gnupg lsb-release

# Добавление официального GPG ключа Docker
sudo mkdir -p /etc/apt/keyrings
curl -fsSL https://download.docker.com/linux/ubuntu/gpg | sudo gpg --dearmor -o /etc/apt/keyrings/docker.gpg

# Добавление репозитория Docker
echo \
  "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.gpg] https://download.docker.com/linux/ubuntu \
  $(lsb_release -cs) stable" | sudo tee /etc/apt/sources.list.d/docker.list > /dev/null

# Установка Docker Engine и Docker Compose V2
sudo apt update
sudo apt install -y docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin

# Запуск и автозапуск Docker
sudo systemctl start docker
sudo systemctl enable docker

# Добавление пользователя в группу docker (чтобы не использовать sudo)
sudo usermod -aG docker $USER

# Выйдите и войдите снова, чтобы изменения вступили в силу
# Или выполните:
newgrp docker
```

**Удаление старой версии docker-compose (если установлена):**

```bash
# Удаление старой версии docker-compose (V1)
sudo apt remove docker-compose  # Ubuntu/Debian
```

**Проверка установки:**

```bash
# Проверка Git
git --version

# Проверка Docker
docker --version

# Проверка Docker Compose V2 (должна быть команда с пробелом)
docker compose version

# Проверка, что Docker работает без sudo
docker ps

# Проверка, что docker-compose (V1) не установлена
docker-compose --version  # Должна выдать ошибку "command not found"
```

**Если команда `docker compose` не работает:**

1. Убедитесь, что установлен пакет `docker-compose-plugin`:
   ```bash
   sudo apt install docker-compose-plugin  # Ubuntu/Debian
   ```

2. Проверьте, что Docker Compose V2 доступен:
   ```bash
   docker compose version
   ```

3. Если все еще не работает, перезапустите Docker:
   ```bash
   sudo systemctl restart docker
   newgrp docker
   ```

### 5.2. Настройка Git на сервере

**Клонирование репозитория (если еще не клонирован):**

```bash
cd ~
git clone <your-repo-url> ym-playlist-bot
cd ym-playlist-bot
```

**Настройка Git для автоматического pull:**

```bash
# Настройка пользователя Git (если еще не настроен)
git config --global user.name "Deploy Bot"
git config --global user.email "deploy@example.com"

# Настройка автоматического pull без конфликтов
git config pull.rebase false
```

### 5.3. Настройка прав доступа

**Убедитесь, что пользователь имеет права на:**
- Чтение/запись в директории проекта
- Запуск Docker команд (без sudo)

```bash
# Добавление пользователя в группу docker (если еще не добавлен)
sudo usermod -aG docker $USER

# Выйдите и войдите снова, чтобы изменения вступили в силу
# Или выполните:
newgrp docker

# Проверка прав
docker ps  # Должно работать без sudo
```

### 5.4. Настройка SSH для CI/CD

**Создание отдельного пользователя для деплоя (рекомендуется):**

```bash
# Создание пользователя
sudo adduser deploy-bot

# Добавление в группу docker
sudo usermod -aG docker deploy-bot

# Настройка SSH ключа
sudo -u deploy-bot mkdir -p /home/deploy-bot/.ssh
sudo -u deploy-bot chmod 700 /home/deploy-bot/.ssh

# Добавление публичного ключа GitHub Actions
sudo -u deploy-bot nano /home/deploy-bot/.ssh/authorized_keys
# Вставьте публичный ключ из ~/.ssh/github_actions_deploy.pub

# Установка прав
sudo -u deploy-bot chmod 600 /home/deploy-bot/.ssh/authorized_keys
```

**Или используйте существующего пользователя:**

Если используете существующего пользователя, убедитесь, что:
- Пользователь имеет доступ к директории проекта
- Пользователь может выполнять Docker команды без sudo
- SSH ключ добавлен в `~/.ssh/authorized_keys`

---

## 6. Ручной запуск деплоя (Manual Trigger)

### 6.1. Запуск через GitHub UI

1. Откройте репозиторий на GitHub
2. Перейдите в **Actions**
3. Выберите workflow **Deploy to Production**
4. Нажмите **Run workflow**
5. Выберите ветку (обычно `main`)
6. Нажмите **Run workflow**

### 6.2. Запуск через GitHub CLI

```bash
# Установка GitHub CLI (если не установлен)
# macOS: brew install gh
# Linux: sudo apt install gh

# Авторизация
gh auth login

# Запуск workflow
gh workflow run deploy.yml
```

### 6.3. Ограничение доступа к ручному запуску

Чтобы ограничить ручной запуск только для владельца репозитория, добавьте проверку в workflow:

```yaml
jobs:
  deploy:
    runs-on: ubuntu-latest
    # Проверка прав доступа для ручного запуска
    if: github.event_name == 'push' || github.event.pull_request.merged == true || github.actor == 'your-username'
    
    steps:
      # ... остальные шаги
```

---

## 7. Автоматическое развертывание при push в main

### 7.1. Настройка триггера

Workflow автоматически запускается при push в ветку `main` благодаря настройке:

```yaml
on:
  push:
    branches:
      - main
```

### 7.2. Защита ветки main (рекомендуется)

Для предотвращения случайных деплоев настройте защиту ветки `main`:

1. Откройте **Settings** → **Branches**
2. Добавьте правило для ветки `main`:
   - ✅ Require pull request reviews before merging
   - ✅ Require status checks to pass before merging
   - ✅ Require branches to be up to date before merging
   - ✅ Include administrators

### 7.3. Использование тегов для версионирования

Для развертывания только определенных версий используйте теги:

```yaml
on:
  push:
    tags:
      - 'v*'  # Развертывание при создании тега v1.0.0, v1.1.0 и т.д.
```

**Создание тега:**

```bash
# Создание тега
git tag -a v1.0.0 -m "Release version 1.0.0"

# Отправка тега в репозиторий
git push origin v1.0.0
```

---

## 8. Процесс деплоя: детали

### 8.1. Сборка Docker-образа

```bash
docker compose build bot
```

**Что происходит:**
- Docker читает `Dockerfile`
- Собирает образ с тегом из `docker-compose.yml`
- Кэширует слои для ускорения последующих сборок

**Оптимизация:**
- Используйте `.dockerignore` для исключения ненужных файлов
- Многоэтапная сборка (multi-stage build) для уменьшения размера образа

### 8.2. Остановка старого контейнера

```bash
docker compose stop bot
```

**Что происходит:**
- Останавливает контейнер `ym_bot`
- Контейнер остается, но не запущен
- Данные в volumes сохраняются

**Альтернатива (полное удаление):**

```bash
docker compose stop bot
docker compose rm -f bot  # Удаление контейнера
```

### 8.3. Запуск нового контейнера

```bash
docker compose up -d bot
```

**Что происходит:**
- Создает новый контейнер из обновленного образа
- Запускает контейнер в фоновом режиме (`-d`)
- Подключает к сети `bot_network`
- Применяет переменные окружения из `.env`

### 8.4. Проверка статуса

```bash
docker compose ps
```

**Ожидаемый результат:**

```
NAME              IMAGE           STATUS
ym_bot            bot:latest      Up 2 minutes
ym_bot_postgres   postgres:15     Up 5 days
ym_bot_pgadmin    pgadmin:8       Up 5 days
```

---

## 9. Откат к предыдущей версии

### 9.1. Автоматическое сохранение версии

Workflow автоматически сохраняет текущую версию в файл `.deploy_version`:

```bash
CURRENT_VERSION=$(git rev-parse --short HEAD)
echo "$CURRENT_VERSION" > .deploy_version
```

### 9.2. Ручной откат через SSH

**Подключение к серверу:**

```bash
ssh user@your-server
cd /path/to/project
```

**Откат к предыдущей версии:**

```bash
# Просмотр истории коммитов
git log --oneline -10

# Откат к конкретному коммиту
git checkout <commit-hash>
docker compose build bot
docker compose stop bot
docker compose up -d bot

# Или откат к предыдущему коммиту
git checkout HEAD~1
docker compose build bot
docker compose stop bot
docker compose up -d bot
```

**Откат к сохраненной версии:**

```bash
# Чтение сохраненной версии
PREVIOUS_VERSION=$(cat .deploy_version)

# Откат к этой версии
git checkout $PREVIOUS_VERSION
docker compose build bot
docker compose stop bot
docker compose up -d bot
```

### 9.3. Откат через GitHub Actions

Создайте отдельный workflow для отката (`.github/workflows/rollback.yml`):

```yaml
name: Rollback to Previous Version

on:
  workflow_dispatch:
    inputs:
      commit_hash:
        description: 'Commit hash to rollback to'
        required: true

jobs:
  rollback:
    runs-on: ubuntu-latest
    
    steps:
      - name: Checkout code
        uses: actions/checkout@v4
      
      - name: Rollback on server
        uses: appleboy/ssh-action@v1.0.0
        with:
          host: ${{ secrets.DEPLOY_HOST }}
          username: ${{ secrets.DEPLOY_USER }}
          key: ${{ secrets.DEPLOY_SSH_KEY }}
          script: |
            cd ${{ secrets.DEPLOY_PATH }}
            git checkout ${{ github.event.inputs.commit_hash }}
            docker compose build bot
            docker compose stop bot
            docker compose up -d bot
            docker compose ps
```

---

## 10. Безопасность

### 10.1. Защита секретов

**Правила:**
- ✅ Никогда не коммитьте секреты в репозиторий
- ✅ Используйте GitHub Secrets для хранения секретов
- ✅ Ограничьте доступ к секретам только необходимым workflow
- ✅ Регулярно ротируйте SSH ключи и токены

### 10.2. Ограничение доступа к workflow

**Настройка ограничений:**

1. Откройте **Settings** → **Actions** → **General**
2. В разделе **Workflow permissions**:
   - Выберите **Read and write permissions** (если нужен доступ к репозиторию)
   - Или **Read repository contents and packages permissions** (только чтение)
3. Отметьте **Allow GitHub Actions to create and approve pull requests** (если нужно)

### 10.3. SSH ключи

**Рекомендации:**
- Используйте отдельный SSH ключ для CI/CD (не личный)
- Ограничьте права ключа только необходимыми командами
- Регулярно ротируйте ключи (каждые 3-6 месяцев)
- Используйте ключи с паролем (passphrase) для дополнительной защиты

**Ограничение команд SSH ключа:**

Создайте файл `~/.ssh/authorized_keys` с ограничениями:

```
command="cd /path/to/project && $SSH_ORIGINAL_COMMAND",no-port-forwarding,no-X11-forwarding,no-agent-forwarding ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAA... github-actions-deploy
```

### 10.4. Firewall и сеть

**Рекомендации:**
- Ограничьте SSH доступ только с IP-адресов GitHub Actions (если возможно)
- Используйте fail2ban для защиты от брутфорса
- Настройте firewall для ограничения доступа к портам

**IP-адреса GitHub Actions:**

GitHub Actions использует динамические IP-адреса. Для ограничения доступа используйте:
- VPN туннель
- GitHub Actions IP ranges (требуется регулярное обновление)

---

## 11. Мониторинг и уведомления

### 11.1. Уведомления в Telegram

**Настройка Telegram бота для уведомлений:**

1. Создайте бота через [@BotFather](https://t.me/BotFather)
2. Получите токен бота
3. Узнайте ваш Chat ID (отправьте сообщение боту [@userinfobot](https://t.me/userinfobot))
4. Добавьте секреты в GitHub:
   - `TELEGRAM_NOTIFICATION_TOKEN`
   - `TELEGRAM_NOTIFICATION_CHAT_ID`

**Использование в workflow:**

```yaml
- name: Send notification
  if: always()  # Отправка при любом результате
  uses: appleboy/telegram-action@v1.0.0
  with:
    to: ${{ secrets.TELEGRAM_NOTIFICATION_CHAT_ID }}
    token: ${{ secrets.TELEGRAM_NOTIFICATION_TOKEN }}
    message: |
      ${{ job.status == 'success' && '✅' || '❌' }} Деплой завершен
      Статус: ${{ job.status }}
      Версия: ${{ github.sha }}
```

### 11.2. Логирование деплоев

**Просмотр логов в GitHub:**

1. Откройте **Actions**
2. Выберите нужный workflow run
3. Нажмите на job **deploy**
4. Просмотрите логи каждого шага

**Логи на сервере:**

```bash
# Логи бота
docker compose logs -f bot

# Логи всех сервисов
docker compose logs -f

# Логи за последний час
docker compose logs --since 1h bot
```

### 11.3. Проверка здоровья приложения

**Добавление health check в workflow:**

```yaml
- name: Health check
  uses: appleboy/ssh-action@v1.0.0
  with:
    host: ${{ secrets.DEPLOY_HOST }}
    username: ${{ secrets.DEPLOY_USER }}
    key: ${{ secrets.DEPLOY_SSH_KEY }}
    script: |
      # Проверка, что контейнер запущен
      if ! docker compose ps bot | grep -q "Up"; then
        echo "❌ Контейнер не запущен!"
        exit 1
      fi
      
      # Проверка логов на ошибки
      if docker compose logs --tail=50 bot | grep -i "error\|exception"; then
        echo "⚠️ Обнаружены ошибки в логах!"
        exit 1
      fi
      
      echo "✅ Health check пройден"
```

---

## 12. Устранение неполадок

### 12.1. Ошибка с passphrase SSH ключа

**Проблема:** `Enter passphrase for (stdin):` или `Error: Command failed: ssh-add -`

**Причина:** SSH ключ защищен паролем (passphrase), а GitHub Actions не может ввести пароль в неинтерактивном режиме.

**Решение:**
1. Создайте новый SSH ключ **БЕЗ passphrase** специально для CI/CD:
   ```bash
   ssh-keygen -t ed25519 -C "github-actions-deploy" -f ~/.ssh/github_actions_deploy -N ""
   ```
   При запросе passphrase просто нажмите Enter (оставьте пустым).

2. Добавьте публичный ключ на сервер:
   ```bash
   cat ~/.ssh/github_actions_deploy.pub >> ~/.ssh/authorized_keys
   chmod 600 ~/.ssh/authorized_keys
   ```

3. Обновите GitHub Secret `DEPLOY_SSH_KEY` новым приватным ключом:
   ```bash
   cat ~/.ssh/github_actions_deploy
   ```
   Скопируйте весь вывод и вставьте в GitHub Secrets.

**Важно:** Для CI/CD всегда используйте ключи без passphrase. Безопасность обеспечивается хранением ключа в GitHub Secrets, а не паролем.

### 12.2. Ошибка подключения по SSH

**Проблема:** `Permission denied (publickey)`

**Решения:**
1. Проверьте, что SSH ключ добавлен в GitHub Secrets (БЕЗ passphrase!)
2. Проверьте, что публичный ключ добавлен в `~/.ssh/authorized_keys` на сервере
3. Проверьте права доступа: `chmod 600 ~/.ssh/authorized_keys` и `chmod 700 ~/.ssh`
4. Проверьте, что пользователь существует и имеет доступ к директории
5. Проверьте, что в workflow не используется `webfactory/ssh-agent` (он не нужен, если ключ без passphrase)

### 12.3. Ошибка: docker: unknown command: docker compose

**Проблема:** `docker: unknown command: docker compose` или `docker compose: command not found`

**Причина:** На сервере установлена старая версия `docker-compose` (V1) или Docker Compose V2 не установлен.

**Решение:**

1. **Проверьте, какая версия установлена:**
   ```bash
   docker compose version  # Должна работать
   docker-compose --version  # Если работает, значит установлена старая версия
   ```

2. **Установите Docker Compose V2:**
   
   **Для Ubuntu/Debian:**
   ```bash
   sudo apt update
   sudo apt install docker-compose-plugin
   ```
   
   **Для CentOS/RHEL:**
   ```bash
   sudo yum install docker-compose-plugin
   ```

3. **Удалите старую версию docker-compose (V1), если установлена:**
   ```bash
   sudo apt remove docker-compose  # Ubuntu/Debian
   # или
   sudo yum remove docker-compose  # CentOS/RHEL
   ```

4. **Проверьте установку:**
   ```bash
   docker compose version
   # Должно вывести версию, например: Docker Compose version v2.24.0
   ```

5. **Если все еще не работает, перезапустите Docker:**
   ```bash
   sudo systemctl restart docker
   newgrp docker
   ```

**Важно:** Проект использует только `docker compose` (V2) с пробелом. Команда `docker-compose` (V1) не поддерживается.

### 12.4. Ошибка: Docker service failed to start

**Проблема:** `Job for docker.service failed because the control process exited with error code`

**Диагностика:**

1. **Проверьте статус сервиса:**
   ```bash
   sudo systemctl status docker.service
   ```

2. **Просмотрите подробные логи:**
   ```bash
   sudo journalctl -xeu docker.service
   ```

3. **Проверьте конфигурацию Docker:**
   ```bash
   sudo dockerd --validate
   ```

**Частые причины и решения:**

**Причина 1: Конфликт с другими контейнерными системами (containerd, podman)**
```bash
# Проверьте, не запущены ли другие контейнерные системы
sudo systemctl status containerd
sudo systemctl status podman

# Если запущены, остановите их
sudo systemctl stop containerd
sudo systemctl disable containerd
```

**Причина 2: Проблемы с cgroup или AppArmor (Ubuntu)**
```bash
# Проверьте AppArmor
sudo systemctl status apparmor

# Если есть проблемы, перезапустите AppArmor
sudo systemctl restart apparmor
```

**Причина 3: Проблемы с конфигурацией Docker**
```bash
# Проверьте файл конфигурации
sudo cat /etc/docker/daemon.json

# Если файл поврежден, создайте резервную копию и удалите
sudo mv /etc/docker/daemon.json /etc/docker/daemon.json.bak
```

**Причина 4: Проблемы с сокетом Docker**
```bash
# Проверьте, не занят ли сокет
sudo lsof /var/run/docker.sock

# Если занят, остановите процесс и перезапустите Docker
sudo systemctl stop docker
sudo rm -f /var/run/docker.sock
sudo systemctl start docker
```

**Причина 5: Проблемы с зависимостями**
```bash
# Переустановите Docker
sudo apt remove docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin
sudo apt autoremove
sudo apt update
sudo apt install docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin
```

**Причина 6: Проблемы с iptables (часто на VPS)**
```bash
# Проверьте iptables
sudo iptables -L

# Если есть проблемы, сбросьте правила (осторожно!)
sudo iptables -F
sudo iptables -X
sudo systemctl restart docker
```

**Общее решение (если ничего не помогло):**

```bash
# Полная переустановка Docker
sudo systemctl stop docker
sudo systemctl disable docker
sudo apt remove docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin
sudo apt autoremove
sudo rm -rf /var/lib/docker
sudo rm -rf /etc/docker

# Установка заново
sudo apt update
sudo apt install docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin
sudo systemctl start docker
sudo systemctl enable docker
```

### 12.5. Ошибка Docker команд

**Проблема:** `permission denied while trying to connect to the Docker daemon socket`

**Решения:**
1. Добавьте пользователя в группу docker: `sudo usermod -aG docker $USER`
2. Выйдите и войдите снова
3. Проверьте права: `docker ps` (должно работать без sudo)
4. Если используете root, убедитесь, что Docker запущен: `sudo systemctl start docker`
5. Проверьте, что сокет существует: `ls -la /var/run/docker.sock`

### 12.6. Ошибка git pull

**Проблема:** `error: Your local changes to the following files would be overwritten by merge`

**Решения:**
1. Используйте `git reset --hard origin/main` вместо `git pull`
2. Или добавьте `git stash` перед `git pull`
3. Или используйте `git fetch` и `git reset --hard origin/main`

### 12.7. Контейнер не запускается

**Проблема:** Контейнер падает сразу после запуска

**Решения:**
1. Проверьте логи: `docker compose logs bot`
2. Проверьте переменные окружения в `.env`
3. Проверьте, что все зависимости установлены
4. Проверьте, что база данных доступна

### 12.8. Медленный деплой

**Проблема:** Деплой занимает слишком много времени

**Решения:**
1. Используйте Docker layer caching
2. Оптимизируйте Dockerfile (многоэтапная сборка)
3. Используйте `.dockerignore` для исключения ненужных файлов
4. Рассмотрите использование Docker registry для кэширования образов

---

## 13. Дополнительные улучшения

### 13.1. Использование Docker Registry

Для ускорения деплоя можно использовать Docker Registry (Docker Hub, GitHub Container Registry):

```yaml
- name: Build and push Docker image
  uses: docker/build-push-action@v5
  with:
    context: .
    push: true
    tags: your-username/ym-playlist-bot:latest
    cache-from: type=registry,ref=your-username/ym-playlist-bot:latest
    cache-to: type=inline

- name: Deploy to server
  uses: appleboy/ssh-action@v1.0.0
  with:
    script: |
      docker pull your-username/ym-playlist-bot:latest
      docker compose up -d bot
```

### 13.2. Blue-Green Deployment

Для нулевого downtime можно использовать blue-green deployment:

```yaml
script: |
  # Запуск нового контейнера с другим именем
  docker compose -f docker-compose.yml -f docker-compose.blue.yml up -d bot-blue
  
  # Проверка здоровья
  # ... health check ...
  
  # Переключение трафика (если используется load balancer)
  # Остановка старого контейнера
  docker compose stop bot
```

### 13.3. Тестирование перед деплоем

Добавьте шаг тестирования перед деплоем:

```yaml
- name: Run tests
  run: |
    # Установка зависимостей
    pip install -r requirements.txt
    
    # Запуск тестов
    pytest tests/
    
    # Проверка линтера
    flake8 .
    black --check .
```

### 13.4. Деплой в staging перед production

Создайте отдельный workflow для staging:

```yaml
# .github/workflows/deploy-staging.yml
name: Deploy to Staging

on:
  push:
    branches:
      - develop

jobs:
  deploy-staging:
    # ... аналогично production, но с другими секретами
```

---

## 14. Чеклист настройки CI/CD

### 14.1. Подготовка

- [ ] Установлен Git на сервере
- [ ] Установлен Docker на сервере
- [ ] Установлен Docker Compose на сервере
- [ ] Репозиторий клонирован на сервер
- [ ] Настроены переменные окружения (`.env`)

### 14.2. Настройка SSH

- [ ] Создан SSH ключ для CI/CD
- [ ] Публичный ключ добавлен в `~/.ssh/authorized_keys`
- [ ] Приватный ключ добавлен в GitHub Secrets (`DEPLOY_SSH_KEY`)
- [ ] Проверено подключение по SSH

### 14.3. Настройка GitHub Secrets

- [ ] `DEPLOY_HOST` — IP-адрес или домен сервера
- [ ] `DEPLOY_USER` — имя пользователя для SSH
- [ ] `DEPLOY_SSH_KEY` — приватный SSH ключ
- [ ] `DEPLOY_PATH` — путь к проекту на сервере
- [ ] `DEPLOY_PORT` — SSH порт (опционально)

### 14.4. Создание workflow

- [ ] Создан файл `.github/workflows/deploy.yml`
- [ ] Настроен триггер для ветки `main`
- [ ] Настроен ручной запуск (`workflow_dispatch`)
- [ ] Добавлены все необходимые шаги

### 14.5. Тестирование

- [ ] Протестирован ручной запуск workflow
- [ ] Проверено автоматическое развертывание при push
- [ ] Проверен откат к предыдущей версии
- [ ] Проверены уведомления (если настроены)

### 14.6. Документация

- [ ] Документирован процесс деплоя
- [ ] Документированы секреты и их назначение
- [ ] Документированы процедуры отката
- [ ] Документированы процедуры устранения неполадок

---

## 15. Примеры использования

### 15.1. Стандартный деплой

```bash
# На локальной машине
git add .
git commit -m "Добавлена новая функция"
git push origin main

# Автоматически запустится workflow и развернет на сервере
```

### 15.2. Ручной деплой

1. Откройте GitHub → **Actions**
2. Выберите **Deploy to Production**
3. Нажмите **Run workflow**
4. Выберите ветку `main`
5. Нажмите **Run workflow**

### 15.3. Откат к предыдущей версии

```bash
# На сервере
ssh user@server
cd /path/to/project
git log --oneline -10  # Просмотр истории
git checkout <commit-hash>  # Откат к нужному коммиту
docker compose build bot
docker compose stop bot
docker compose up -d bot
```

---

## 16. Заключение

Настройка CI/CD для автоматического развертывания значительно упрощает процесс обновления бота и снижает риск ошибок при ручном деплое.

**Преимущества:**
- ✅ Автоматизация процесса развертывания
- ✅ Быстрое развертывание новых версий
- ✅ Возможность отката к предыдущей версии
- ✅ Безопасное хранение секретов
- ✅ История всех деплоев в GitHub Actions

**Следующие шаги:**
1. Настройте CI/CD согласно этой документации
2. Протестируйте процесс деплоя
3. Настройте мониторинг и уведомления
4. Документируйте специфичные для вашего проекта детали

---

**Документ создан:** 2025-12-02  
**Версия:** 1.0  
**Автор:** AI Agent (на основе анализа кодовой базы и best practices CI/CD)

