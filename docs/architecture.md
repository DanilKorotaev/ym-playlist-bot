...
- **Парсинг ссылок**: Встроен в `bot_v2.py`
...
- **Опциональные**: `PLAYLIST_OWNER_ID`, `PLAYLIST_ID`, `PLAYLIST_KIND` (для обратной совместимости)

### Структура проекта
```
ym-playlist-bot/
├── bot_v2.py              # Основной файл бота (активная версия)
├── bot.py                 # Старая версия (один плейлист)
├── database.py            # Работа с SQLite БД
├── yandex_client_manager.py  # Управление клиентами Яндекс.Музыки
├── migrate_old_playlist.py  # Скрипт миграции старого плейлиста
└── docs/                  # Документация
```
