...
### Рефакторинг
- **Унификация отправки сообщений**: Создать утилиту для унифицированной отправки сообщений, устранить дублирование между `update.callback_query.message.reply_text` и `update.effective_message.reply_text`
- **Общие ответы**: Вынести повторяющиеся сообщения (например, "❌ У вас нет активного плейлиста") в константы или утилиту
- **Разбиение больших методов**: Рефакторинг `button_callback` в `handlers/callbacks.py` - вынести каждый `if` блок в отдельный метод
