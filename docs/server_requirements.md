...
### 4.5. Синхронная архитектура

**Проблема:**
- Текущая версия использует синхронный `python-telegram-bot==13.15`
- Блокирующие операции (API запросы, БД) замедляют обработку других запросов

**Решения:**
- Миграция на `aiogram 3.x` (асинхронная версия) - запланировано в TODO
- Асинхронные операции с БД (async SQLAlchemy)
- Асинхронные HTTP запросы (aiohttp)

**Рекомендации:**
- Приоритет: средний (важно для масштабирования, но не критично для <500 пользователей)
- После миграции ожидается улучшение производительности на 30-50%
