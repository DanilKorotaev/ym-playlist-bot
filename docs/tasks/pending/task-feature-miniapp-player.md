...
### Этап 1: Инфраструктура и базовая структура (MVP)

- [ ] Создать структуру папок `miniapp/static/`
- [ ] Создать базовый `index.html` с подключением Telegram Web App API
- [ ] Создать `miniapp/static/js/telegram-api.js` - обертка для Telegram API
- [ ] Настроить локальную разработку (HTTP сервер + ngrok для HTTPS)
- [ ] Добавить кнопку Web App в главное меню бота
- [ ] Создать обработчик `handlers/webapp.py` для приема данных от Mini App
- [ ] Протестировать базовое взаимодействие (отправка/получение данных)

**Результат:** Mini App открывается, может отправлять и получать данные от бота.

### Этап 2: Базовый плеер

- [ ] Создать `miniapp/static/js/player.js` - класс для управления воспроизведением
- [ ] Реализовать загрузку трека через Fetch API с Range Requests
- [ ] Реализовать создание Blob URL с правильным Content-Type
- [ ] Реализовать базовое воспроизведение (play/pause)
- [ ] Добавить UI элементы (кнопки play/pause, прогресс-бар)
- [ ] Создать `miniapp/static/css/styles.css` с базовыми стилями
- [ ] Интегрировать с ботом: получение URL трека через `YandexService`

**Результат:** Можно воспроизводить один трек из плейлиста.

### Этап 3: Работа с плейлистами

- [ ] Создать `miniapp/static/js/playlist.js` - работа с плейлистами
- [ ] Реализовать получение списка доступных плейлистов от бота
- [ ] Реализовать выбор плейлиста в UI
- [ ] Реализовать получение списка треков из выбранного плейлиста
- [ ] Реализовать отображение списка треков в Mini App
- [ ] Реализовать выбор трека для воспроизведения
- [ ] Интегрировать с `PlaylistService` для получения данных

**Результат:** Можно выбрать плейлист и трек для воспроизведения.

### Этап 4: Очередь воспроизведения

- [ ] Реализовать очередь треков (массив треков)
- [ ] Реализовать автоматический переход к следующему треку
- [ ] Реализовать кнопки "Следующий" / "Предыдущий"
- [ ] Реализовать отображение текущего трека в очереди
- [ ] Реализовать индикатор прогресса воспроизведения
- [ ] Добавить отображение названия трека и артистов

**Результат:** Полноценная очередь воспроизведения с навигацией.

### Этап 5: Автоматическое обновление очереди

- [ ] Реализовать периодическую проверку обновлений плейлиста (polling)
  - **Интервал:** 10 секунд (настраивается через конфиг)
  - **Умная проверка:** Проверка выполняется только когда текущий трек близок к концу очереди (последние 2-3 трека)
  - **Оптимизация:** Если очередь не близка к концу, проверка не выполняется
- [ ] Реализовать сравнение текущей очереди с актуальным состоянием плейлиста (через revision)
- [ ] Реализовать автоматическое добавление новых треков в конец очереди
- [ ] Реализовать уведомления о новых треках (toast-уведомление в UI)
- [ ] Реализовать эндпоинт `/api/playlists/{id}/updates` для проверки обновлений
...
- [ ] Добавить темную тему (соответствующую Telegram)
- [ ] Добавить отображение обложек треков/плейлистов
- [ ] Добавить управление громкостью
- [ ] Добавить перемотку трека (seek)
- [ ] Добавить режимы воспроизведения (повтор, перемешивание)
